...
for k=1:length(dtVec(:,1))
    dt=dtVec(k,1);
    data = zeros(length(0:dt:100*Tp),5);
    data(:,1) = 0:dt:100*Tp;        %First Coloumn dt
    ...
    for i=2:length(data(:,1))
        ...
        data(i,5) = sqrt(2*epsilonSi*data(i,2)/(q*Na));
...
%         end
        tempi=i;
        if (data(i,4)>1.34*Q_iSteadyState)  
           break;
        end
    end
        figure;
        ...
        subplot(2,2,2);
        plot(data(1:tempi,1),data(1:tempi,2),'--k');
        xlabel('Time(s)');
        ylabel('Surface Potential (V)');
        title('SurfacePotential vs. Time(t)');
        grid on;
        
        subplot(2,2,3);
        plot(data(1:tempi,1),data(1:tempi,3),'--k');
        xlabel('Time(s)');
        ylabel('Q_d_e_p (C)');
        title('Q_d_e_p vs. Time(t)');
        grid on;
        
        subplot(2,2,4);
        plot(data(1:tempi,1),data(1:tempi,5),'--k');
        xlabel('Time(s)');
        ylabel('Width (cm)');
        title('Width vs. Time(t)');
        grid on;
